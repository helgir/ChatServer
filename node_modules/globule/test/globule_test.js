...
exports['match'] = {
    ...
    'exclusion': function(test) {
        ...
        test.deepEqual(globule.match(['*.js', '!f*.js'], ['foo.js', 'bar.js', 'baz.js']), ['bar.js', 'baz.js'],
            'partial exclusion should partially cancel match');
        test.deepEqual(globule.match(['*.js', '!*.js', 'b*.js'], ['foo.js', 'bar.js', 'baz.js']), ['bar.js', 'baz.js'],
            'inclusion / exclusion order matters');
        test.deepEqual(globule.match(['*.js', '!f*.js', '*.js'], ['foo.js', 'bar.js', 'baz.js']), ['bar.js', 'baz.js', 'foo.js'],
            'inclusion / exclusion order matters');
        test.done();
    },
    'options.matchBase': function(test) {
        ...
        test.deepEqual(globule.match('*.js', ['foo.js', 'bar', 'baz/xyz.js']), ['foo.js'],
...
exports['isMatch'] = {
    ...
    'options.matchBase': function(test) {
        test.expect(2);
        test.ok(globule.isMatch('*.js', ['baz/xyz.js'], {
            matchBase: true
        }), 'should matchBase (minimatch) when specified.');
...
exports['find'] = {
    ...
    'basic matching': function(test) {
        ...
        test.deepEqual(globule.find('**/*.js', '**/*.css'), ['js/bar.js', 'js/foo.js', 'css/baz.css', 'css/qux.css'],
    ...
    'exclusion': function(test) {
        ...
        test.deepEqual(globule.find(['**/*.js', '!js/foo.js']), ['js/bar.js'],
            'should omit single file from matched set');
        test.deepEqual(globule.find(['!js/foo.js', '**/*.js']), ['js/bar.js', 'js/foo.js'],
            'inclusion / exclusion order matters');
        test.deepEqual(globule.find(['**/*.js', '**/*.css', '!js/bar.js', '!css/baz.css']), ['js/foo.js', 'css/qux.css'],
            'multiple exclusions should be removed from the set');
        test.deepEqual(globule.find(['**/*.js', '**/*.css', '!**/*.css']), ['js/bar.js', 'js/foo.js'],
            'excluded wildcards should be removed from the matched set');
        test.deepEqual(globule.find(['js/bar.js', 'js/foo.js', 'css/baz.css', 'css/qux.css', '!**/b*.*']), ['js/foo.js', 'css/qux.css'],
            'different pattern for exclusion should still work');
        test.deepEqual(globule.find(['js/bar.js', '!**/b*.*', 'js/foo.js', 'css/baz.css', 'css/qux.css']), ['js/foo.js', 'css/baz.css', 'css/qux.css'],
            'inclusion / exclusion order matters');
        test.done();
    },
    ...
    'options.filter': function(test) {
        ...
        test.deepEqual(globule.find('**/*.js', {
            filter: 'isDirectory'
        }), [], 'should fail to match.');
        test.done();
    },
    ...
    'options.srcBase': function(test) {
        ...
        test.deepEqual(globule.find(['**/deep*'], {
            ...
            'srcBase should not prevent filtering.');
        test.deepEqual(globule.find(['**/deep*.txt', '!**/deeper**'], {
                srcBase: 'deep'
    ...
    'options.nonull': function(test) {
        test.expect(3);
        test.deepEqual(globule.find(['*omg*'], {
                nonull: true
            }), ['*omg*'],
            'non-matching patterns should be returned in result set.');
        test.deepEqual(globule.find(['js/a*', 'js/b*', 'js/c*'], {
                nonull: true
            }), ['js/a*', 'js/bar.js', 'js/c*'],
            'non-matching patterns should be returned in result set.');
        test.deepEqual(globule.find(['js/foo.js', 'js/bar.js', 'js/nonexistent.js'], {
                nonull: true
...
exports['mapping'] = {
    ...
    'options.ext': function(test) {
        ...
        actual = globule.mapping(['x/a.js', 'x.y/b.min.js', 'x.y/z.z/c'], {
            ext: '.foo'
        });
        expected = [{
            dest: 'x/a.foo',
