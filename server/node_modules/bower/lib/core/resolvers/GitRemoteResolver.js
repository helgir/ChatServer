...
GitRemoteResolver.prototype._slowClone = function(resolution) {
    return cmd('git', ['clone', this._source, this._tempDir, '--progress'])
        .then(cmd.bind(cmd, 'git', ['checkout', resolution.commit], {
            cwd: this._tempDir
        }));
};
...
GitRemoteResolver.prototype._supportsShallowCloning = function() {
    ...
    // shallow cloning.
    if (mout.string.startsWith(this._remote.protocol, 'http') && !GitRemoteResolver._canShallow.get(this._host)) {
...
GitRemoteResolver._canShallow = new LRU({
    max: 50,
    maxAge: 5 * 60 * 1000
});
