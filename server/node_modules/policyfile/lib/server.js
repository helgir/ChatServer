    ...
    this.socket.on('error', function serverError(err) {
        ...
        if (err.errno == 13) {
            me.log && me.log(
                ...
                (
                    me.server ? 'The Flash Policy File requests will only be served inline over the supplied HTTP server. Inline serving is slower than a dedicated server instance.' : 'No fallback server supplied, we will be unable to answer Flash Policy File requests.'
                )
            );
    ...
    if (this.server) {
        ...
        this.server['@'] = function connection(socket) {
            socket.once('data', function requestData(data) {
                ...
                if (
                    data && data[0] === 60 && data.toString() === '<policy-file-request/>\0' && socket && (socket.readyState === 'open' || socket.readyState === 'writeOnly')
                ) {
                    // send the buffer
                    try {
...
Server.prototype.add = function add() {
    ...
    Array.prototype.push.apply(
        this.origins, args.filter(function filter(value) {
            return !!value;
        })
    );
